...
use aidoku::{
    Chapter, ContentRating, DeepLinkHandler, DeepLinkResult, DynamicSettings, HashMap,
    ImageRequestProvider, Listing, ListingProvider, Manga, MangaPageResult, MangaStatus, Page,
    PageContent, Result, Setting, Source, TextSetting, UpdateStrategy, Viewer,
    alloc::{
        String, Vec,
        borrow::Cow,
        format,
...
use parser::{GalleryKey, parse_gallery_key};
...
struct ApiGalleryItem {
    ...
    pages: Option<u32>,
    tag: Option<Vec<ApiTag>>,
    lowtag: Option<Vec<ApiTag>>,
}

#[derive(Debug, Deserialize)]
struct SearchMeta {
    #[serde(rename = "Filtered")]
    _filtered: Option<String>,
    #[serde(rename = "Found")]
    _found: Option<String>,
    _about: Option<bool>,
}

#[derive(Debug, Deserialize)]
struct FavoriteListingResponse {
    index: StandardListingResponse,
    _fav: Option<Vec<String>>,
}
...
impl Source for ExMirrorApi {
    ...
    fn get_search_manga_list(
        ...
        let base_api = match query.filter(|value| !value.trim().is_empty()) {
            Some(query) => format!("/api/i?f_search={}", encode_query_value(&query)),
            None => "/api/i".into(),
        };
        self.get_manga_page_from_api(base_api, page)
    }
...
impl ListingProvider for ExMirrorApi {
    fn get_manga_list(&self, listing: Listing, page: i32) -> Result<MangaPageResult> {
        let path = match listing.id.as_str() {
            "popular" => "/api/popular",
            "watched" => "/api/watched",
            "favorites" => "/api/favorites",
            _ => return Err(AidokuError::message("Unknown EXMirror API listing")),
        };
        self.get_manga_page_from_api(path.into(), page)
    }
}

impl DynamicSettings for ExMirrorApi {
    fn get_dynamic_settings(&self) -> Result<Vec<Setting>> {
        Ok(vec![
            ...
            text_setting("sk", "sk", "Required for EXMirror API listings", true),
        ])
    }
}

impl ImageRequestProvider for ExMirrorApi {
...
impl DeepLinkHandler for ExMirrorApi {
...
impl ExMirrorApi {
    fn get_manga_page_from_api(&self, base_api: String, page: i32) -> Result<MangaPageResult> {
        require_listing_cookie()?;
        let page = core::cmp::max(page, 1);
        let path = if page <= 1 {
            base_api.clone()
        } else {
            match defaults_get::<String>(&next_cache_key(&base_api, page)) {
                Some(path) => path,
                None => {
        ...
        let listing = request_api_listing(&path)?;
        let mut entries = Vec::new();
        for item in &listing.items {
            if let Some(manga) = manga_from_api_item(item) {
                entries.push(manga);
            }
        }
        if entries.is_empty() {
        ...
        let next_path = listing
            .next
            .as_ref()
            .and_then(|url| next_api_path(&path, url));
        if let Some(next_path) = &next_path {
            defaults_set(
                &next_cache_key(&base_api, page + 1),
                DefaultValue::String(next_path.clone()),
            );
        }

        Ok(MangaPageResult {
            entries,
            has_next_page: next_path.is_some(),
        })
    }
...
fn request_api_listing(path: &str) -> Result<ApiListingPage> {
    let endpoint = api_endpoint(path);
    let response = request_with_cookie(&absolute_url(path))?.send()?;
    ...
    if status != 200 {
        return Err(AidokuError::message(format!(
            "EXMirror API {} returned HTTP {} (content-type: {}). This source only uses /api/*; verify the API accepts your cookies.",
            endpoint, status, content_type
        )));
    }
    if endpoint == "/api/favorites" {
        let response: FavoriteListingResponse = response.get_json_owned()?;
        let (items, _prev, next, _meta) = response.index;
        Ok(ApiListingPage { items, next })
    } else {
        let (items, _prev, next, _meta): StandardListingResponse = response.get_json_owned()?;
        Ok(ApiListingPage { items, next })
    }
}

fn manga_from_api_item(item: &ApiGalleryItem) -> Option<Manga> {
    let key = parse_gallery_key(&item.href)?;
    let tags = api_item_tags(item);
    Some(Manga {
        key: key.as_key(),
        ...
        cover: Some(normalize_gallery_image_url(&item.src)),
        url: Some(format!("{}/g/{}/{}/", BASE_URL, key.gid, key.token)),
        artists: item.uploader.clone().map(|value| vec![value.clone()]),
        authors: item.uploader.clone().map(|value| vec![value]),
        description: description_from_api_item(item),
...
fn api_item_tags(item: &ApiGalleryItem) -> Vec<String> {
    let mut tags = Vec::new();
    push_non_empty(&mut tags, item.catalog.as_ref());
    for value in item.tag.iter().flatten() {
        push_non_empty(&mut tags, api_tag_name(value).as_ref());
    }
    for value in item.lowtag.iter().flatten() {
        push_non_empty(&mut tags, api_tag_name(value).as_ref());
    }
    tags
}

fn api_tag_name(tag: &ApiTag) -> Option<String> {
    tag.tr
        .as_ref()
        ...
        .or_else(|| {
            if tag.title.trim().is_empty() {
                ...
                Some(tag.title.clone())
            }
        })
}

fn description_from_api_item(item: &ApiGalleryItem) -> Option<String> {
    let mut lines = Vec::new();
    if let Some(time) = &item.time {
        lines.push(format!("Posted: {}", time));
    }
    if let Some(pages) = item.pages {
        lines.push(format!("Pages: {}", pages));
    }
    if let Some(catalog) = &item.catalog {
        lines.push(format!("Category: {}", catalog));
    }
    if lines.is_empty() {
        None
    } else {
...
fn request_with_cookie(url: &str) -> Result<Request> {
    ...
    if url.starts_with(&format!("{}/api/", BASE_URL)) {
...
fn require_listing_cookie() -> Result<()> {
    optional_setting("sk")
        .map(|_| ())
        .ok_or_else(|| AidokuError::message("Missing EXMirror setting: sk. EXMirror API listings require sk; paste the full cookie string including sk."))
}
...
fn extract_cookie_value(key: &str, raw: &str) -> Option<String> {
    ...
    let value = raw[index..].split(';').next().unwrap_or_default().trim();
...
fn mirror_path(url: &str) -> String {
    ...
    for prefix in [BASE_URL, "https://exhentai.org", "https://e-hentai.org"] {
...
fn api_endpoint(path: &str) -> &str {
    path.split('?').next().unwrap_or(path)
}

fn next_api_path(current_api_path: &str, next_url: &str) -> Option<String> {
    if next_url.trim().is_empty() {
        return None;
    }
    let query = query_part(next_url)?;
    let endpoint = api_endpoint(current_api_path);
    if query.is_empty() {
        Some(endpoint.into())
    } else {
        Some(format!("{}?{}", endpoint, query))
    }
}

fn query_part(url: &str) -> Option<String> {
    let query = url.split('?').nth(1)?.split('#').next().unwrap_or_default();
    Some(query.into())
}
...
mod tests {
    ...
    #[aidoku_test]
    fn converts_next_url_to_current_api_endpoint() {
        assert_eq!(
            next_api_path("/api/popular", "https://exhentai.org/popular?next=123"),
            Some("/api/popular?next=123".into())
        );
        assert_eq!(
            next_api_path(
                "/api/i?f_search=artist%3Atest",
                "https://exhentai.org/?f_search=artist%3Atest&next=456"
            ),
            Some("/api/i?f_search=artist%3Atest&next=456".into())
        );
    }
    ...
    fn builds_single_page_api_url_from_exhentai_page_url() {
        assert_eq!(
            page_api_url("https://exhentai.org/s/abc123/3922170-1?x=1").unwrap(),
            (
                "https://ex.elysia.rip/api/s/abc123/3922170-1".into(),
                "https://ex.elysia.rip/s/abc123/3922170-1".into()
            )
    ...
    #[aidoku_test]
    fn extracts_cookie_values_from_full_cookie_string() {
        assert_eq!(
            extract_cookie_value(
                "ipb_pass_hash",
                "ipb_member_id=123; ipb_pass_hash=abc; igneous=mystery"
            ),
            Some("abc".into())
        );
    }
    ...
    #[aidoku_test]
    fn normalizes_s_exhentai_thumbnail_hosts() {
        assert_eq!(
            normalize_gallery_image_url("https://s.exhentai.org/t/abc.jpg"),
            "https://ehgt.org/t/abc.jpg"
        );
    }
}
